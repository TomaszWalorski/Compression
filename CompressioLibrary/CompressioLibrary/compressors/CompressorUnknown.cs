...
{
    class CompressorUnknown : ICompressorUnknown
    {
        ...
        private void SaveFileFormat(string filePath)
        {
            List<char> fileFormat = new List<char>();
            for (int i = 0; i < filePath.Length; i++)
            {
                char currentChar = filePath[filePath.Length - 1 - i];
                fileFormat.Insert(0, currentChar);
                if (currentChar == '.') break;
            }
            this.fileFormat = new String(fileFormat.ToArray());
        }
        ...
        public CompressionOutput compress(string path)
        {
            ...
            var compressedFile = new CompressedFile(EnumDataType.unknown, isCompressed, "compressedFile", isCompressed ? compressedBytes : originalFile);
        ...
        public StringBuilder decompress(CompressedFile compressedFile)
        {
            ...
            else
            {
                ...
                {
                    sw.Write(compressedFile.compressedFile);
